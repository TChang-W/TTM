---
title: "Introduction to TTM"
output: rmarkdown::pdf_document
vignette: >
  %\VignetteIndexEntry{Introduction to TTM}
...
Test the difference in the slope of $\beta_\mu(t*)$ for between and after the knots specified. new_times should be a vector of three retrospective times, where the first one is before the knot and the last is after the knot. The middle one is the knot in the `TTM_linear()` function.
```{r}
test_beta_slope(new_times = c(5, 6, 7), fit = fit_linear, var_name = "X0")
```
Test the difference in the slope of $\beta_A(t*)$ for between and after the knots specified.
```{r}
