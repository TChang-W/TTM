TTM_knots_linear <- function(dat, corstr, knots, spline_type,
                             jackknife = FALSE, jk_block_size){
  time_grid <- dat$t_retro
  if(is.null(knots)){
    knots = median(time_grid)
  }
  data_spline <- switch(spline_type,
    "ns" = splines::ns(time_grid, knots = knots, degree = 1),
    "bs" = splines::bs(time_grid, knots = knots, degree = 1)
  )
